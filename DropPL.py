...
def NN(x,weights,biases,training):
...
#    print( 'HL:', HL.get_shape(), 'W2:', weight['out'].get_shape(), 'b2:', bias['out'].get_shape())        
    # The sigmoid is computed by the function tf.nn.sigmoid_cross_entropy_with_logits
    out_layer = tf.add(tf.matmul(HL, weights['out']), biases['out']) # Out=(HL*h1)+b1
...
pred1 = NN(PLx, weights, biases,training)
...
#sigmoid
Loss = tf.add(tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=pred, labels=y)),tf.multiply(alpha,tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=pred1, labels=PLy))))
...
with tf.Session() as sess:
    ...
    for epoch in range(trainingEpochs):
        ...
        for i in range(total_batch):
            ...
            tr=0
            batch_ypred = sess.run([pred], feed_dict={x: batch_xpred,training: tr})
...
#        print("Optimization Finished!")
    ...
    print (accuracy.eval({x: mnist.test.images, y: mnist.test.labels, training:tr}))
